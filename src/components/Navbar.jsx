  ...
  return (
    <div>
      ...
      <nav className="flex justify-between items-center py-4 px-8 md:px-16 bg-white/80 backdrop-blur-sm shadow-sm relative">
        ...
        <div
          onClick={() => navigate("/")}
          className="flex hover:scale-110 transition-transform items-center cursor-pointer"
        >
          <img
            src={ecoLogo}
            alt="EcoGenie Logo"
          ...
          />
          <p className="font-serif text-xl font-bold text-green-700 ml-2">
            ECOGENIE
          </p>
        </div>
        {/* Centered Menu */}
        ...
        <div className="md:hidden flex flex-col items-center bg-white shadow-md p-4 space-y-4">
          {[
            ...
            <Link
              key={index}
              to={item.href}
              className={`font-medium transition-colors  z-50 ${
                location.pathname === item.href
                  ? "text-green-700 font-semibold"
                  : "text-gray-800 hover:text-green-600"
              }`}
