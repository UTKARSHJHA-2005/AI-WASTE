...
const ProcessorsPage = () => {
  ...
  return (
    <div className="bg-gradient-to-b from-green-50 to-blue-50 min-h-screen overflow-x-hidden">
      ...
      <Navbar />
      {/* Page Header */}
      <motion.div className="container mx-auto px-8 py-12" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }}>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          <div>
            <motion.h1 className="text-4xl font-bold text-green-700" variants={fadeIn} initial="hidden" animate="visible">
          ...
          {userLocation && (
            <motion.div className="flex items-center mt-4 md:mt-0 bg-white px-4 py-2 rounded-lg shadow-sm" initial={{ opacity: 0, x: 20 }}
        ...
        {showContactForm && selectedProcessor && (
          <motion.div className="bg-white rounded-xl shadow-md p-6 mb-8 border-l-4 border-green-600" initial={{ opacity: 0, height: 0 }}
            ...
            <div className="flex justify-between items-start mb-6">
              ...
              <motion.button onClick={() => setShowContactForm(false)} className="text-gray-500 hover:text-gray-700" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
                ✕
              </motion.button>
            </div>
            <form onSubmit={handleSubmitContactForm}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                  ...
                  <select className="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none"
                  required>
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea className="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none h-32"
                  placeholder="Describe your waste management needs, questions, or preferred contact times..." required/>
                </div>
              </div>
              <div className="flex justify-between items-center">
                <div className="flex items-center">
                  <input type="checkbox" id="consent" className="mr-2 h-4 w-4 text-green-600 focus:ring-green-500 border-gray-300 rounded" required/>
        ...
        <motion.div className="bg-white rounded-xl shadow-sm p-6 mb-8" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4">
            <div className="flex items-center w-full md:w-auto mb-4 md:mb-0">
              ...
              <motion.button className="ml-2 bg-green-700 text-white px-4 py-3 rounded-lg" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                Search
              </motion.button>
            </div>
            <div className="flex items-center w-full md:w-auto">
              ...
              <motion.button className="text-green-700" onClick={() => setIsFilterOpen(!isFilterOpen)} whileHover={{ scale: 1.05 }}>
                <ChevronDown className={`transition-transform ${isFilterOpen ? 'rotate-180' : ''}`} />
              </motion.button>
            </div>
          </div>
          {isFilterOpen && (
            <motion.div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}
            transition={{ duration: 0.3 }}>
              ...
              <div>
                ...
                <select className="w-full border border-gray-200 rounded-lg p-2 focus:border-green-500 focus:ring-1 focus:ring-green-500 outline-none"
                value={selectedDistance} onChange={(e) => setSelectedDistance(e.target.value)}>
        ...
        {/* Results Count */}
        <motion.div className="mb-6 text-gray-700" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}>
          Found {filteredProcessors.length} waste processors{location ? ` near "${location}"` : ''}
        </motion.div>
        {/* Processor Cards */}
        <motion.div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-16" variants={staggerChildren} initial="hidden" animate="visible">
          {filteredProcessors.map((processor) => (
            <motion.div key={processor.id} className="bg-white rounded-xl shadow-sm overflow-hidden border border-green-100" variants={fadeIn}
              whileHover={hoverScale.hover}>
              <div className="grid grid-cols-1 md:grid-cols-3">
                <div className="md:col-span-1">
                  <img src={processor.image} alt={processor.name} className="w-full h-full object-cover"/>
                </div>
                <div className="p-5 md:col-span-2">
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-3">
                    <motion.button className="w-full sm:w-auto flex items-center justify-center bg-green-700 text-white px-4 py-2 rounded-lg"
                    onClick={() => openContactForm(processor)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
        ...
        <motion.div className="bg-gradient-to-r from-green-100 to-blue-100 rounded-xl p-8 mb-16" initial={{ opacity: 0, y: 30 }}
          ...
          <div className="flex flex-col md:flex-row items-center justify-between">
            ...
            <motion.button className="bg-green-700 text-white px-6 py-3 rounded-lg flex items-center shadow-md" whileHover={{ scale: 1.05, boxShadow: "0 10px 15px -3px rgba(0, 0, 0, 0.1)" }}
            whileTap={{ scale: 0.97 }} onClick={() => navigate("/join-network")}>
        ...
        <motion.div className="bg-white rounded-xl shadow-md p-6 mb-16" initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7, delay: 0.9 }} data-aos="fade-up">
          <div className="flex justify-between items-center mb-6">
            ...
            <motion.button className="text-green-700 flex items-center" whileHover={{ x: 5 }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <motion.div key={index} className="bg-gradient-to-b from-green-50 to-blue-50 rounded-lg p-5 border border-green-100"
                ...
                <div className="flex justify-between items-center">
                  ...
                  <motion.button className="text-green-700 font-medium text-sm" whileHover={{ scale: 1.05 }}>
                    Learn More
                  </motion.button>
                </div>
              </motion.div>
